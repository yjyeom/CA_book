...
\begin{salt_theorem} \label{thm-4-7}
...
\begin{salt_example} \label{example-4-11}
...
&= \underbrace{\dfrac1{2\pi i} \int_{\gamma_2} \dfrac{f(\zeta)}{\zeta-z}d\zeta}_{(\rm I)}
- \underbrace{\dfrac1{2\pi i} \int_{\gamma_1} \dfrac{f(\zeta)}{\zeta-z}d\zeta}_{(\rm II)}.
\end{align*}
이제 적분 $(\rm I)$은
\[
\sum_{n=0}^\infty c_n(z-z_0)^n
...
원환 $\tilde{\mathbb A_1}:=\{ z\in\mathbb C\,:\, 0 <|z-1|<1\}$과
원환 $\tilde{\mathbb A_2}:=\{ z\in\mathbb C\,:\, 1<|z-1|\}$에서
$f$의 로랑 급수를 구하라.
\end{salt_exercise}
...
\begin{salt_definition} \label{def-4-4}
...
\begin{salt_example}\label{example-4-13}
예를 들어 다음 함수
\[
\dfrac{\sin z}{z}, \quad \dfrac1{z^3}, \quad \exp \dfrac1z
\]
...
\begin{salt_definition} \label{def-4-5}
$f$의 고립 특이점 $z_0$는
\begin{itemize}
...
\begin{salt_example}\label{example-4-14}
\
\begin{itemize}
...
\begin{salt_example}\label{example-4-16}
\
\begin{itemize}
\item[(1)] $z\ne0$에 대하여
\[
...
\begin{salt_theorem} [카소라티-바이어스트라스]
\footnote{
...
\begin{salt_example}\label{example-4-17}
...
\begin{salt_example}\label{example-4-18}
실적분
\[
...
\begin{salt_example}\label{example-4-19}
...
\begin{salt_example} [프레넬 적분]\footnote{
...
\item[(2)] $f_n\le 3^n$로부터 $F$의 수렴반경이 적어도 $1/2$ 이상임을 유도하라.
