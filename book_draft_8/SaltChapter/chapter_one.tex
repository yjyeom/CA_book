...
{\bf 복소수}\index{복소수(complex number)}는 실수의 순서쌍으로 정의한다. 예를 들면,
$$
...
실수 $x$는 $z$의 \textbf{실수부}\index{실수부(real part)}, $y$는 $z$의 \textbf{허수부}\index{허수부(imaginary part)}라고 한다.
...
$(\mathbb C, +, \cdot)$는 \textbf{체}(field)\index{체(field)}이다.
\end{saltprop}
...
카르다노(Cardano, 1501-1576)\index{카르다노(Cardano)}는 
...
$$
카르다노 공식이 나온지 30년 후, 봄벨리(Bombelli)\index{봄벨리(Bombelli)}가 복소수 연산을 도입하면
...
다음 성질을 만족하는 체 $F$의 부분집합 $P$가 있으면, $F$를 순서체\index{순서체(ordered field)}라고 하고, $P$를 $F$의 양의 부분집합이라 한다.
%양의 부분집합 $P\subset \mathbb F$가 있어 다음을 만족하면
%체 $\mathbb F$는 순서(order)\index{순서를 갖는 체(ordered field)}를 갖는다고 한다.
...
\index{복소평면(complex plane)}
복소평면은  아르간트(Argrand)\footnote{ 
실제로 Caspar Wessel (1745-1818)이 더 먼저 사용했지만,
Jean-Robert Argand (1768-1822)의 이름을 따온 것이다.} \index{아르간트 평면(Argand plane)}
평면으로도 불린다.
...
$$
가 성립하며 이를 드 므와브르 정리\index{드 므와브르 정리(de Moivre's formula)}라 한다.
...
가우스 정수(Gaussian integer)\index{가우스 정수(Gaussian integer)}는 
...
드 므와브르 공식을 이용하면
복소수  $z$의 $n$\textbf{제곱근}\index{제곱근(root of a complex number)}
...
이항정리\index{이항정리(binomial theorem)}에서
...
k=0,1,2,\ldots, n,
$$
는 이항계수\index{이항계수(binomial coefficient)}라 한다.
...
{\bf 절대값과 켤레복소수: }
복소수 $z=x+iy$ ($x,y\in\mathbb R$)의 절대값\index{절대값(absolute value)} $|z|$는
$$
|z| = \sqrt{x^2+y^2}
$$
로 정의한다.
...
복소수 $z=x+iy$ ($x,y\in\mathbb R$)의 켤레복소수\index{켤레복소수(complex conjugate)} $\bar z$는
$$
\bar z = x - iy
$$
로 정의한다.
...
다음과 같이 복소수 절대값의 삼각 부등식\index{삼각 부등식(triangle inequality)}을 얻는다.
$$
...
$\mathbb C$가 완비성(completeness)\index{완비성(completeness)}을 가짐을 증명하라.
즉, $\mathbb C$의 모든 코시 수열이 $\mathbb C$의 원소로 수렴한다.
(임의의 $\epsilon>0$에 대하여  $N\in\mathbb N$이 존재하여
$m,n>N$ 이면 $|z_n - z_m| < \epsilon$을 만족할 때
수열 $(z_n)_{n\in\mathbb N}$을 {\bf 코시 수열}\index{코시 수열(Cauchy sequence)}이라 한다.)
\end{salt_exercise}
...
\textbf{계단식 경로}(stepwise path)\index{계단식 경로(stepwise path)}라 한다.
...
$U$를 \textbf{경로연결된}(path-connected)\index{경로연결된(path-connected)} 열린 집합이라 한다.
\end{itemize}
\end{saltdefinition}
...
\index{복소 지수함수(complex exponential function)}
...
확장하는 유일한 방법이기 때문이다. 
이와 관련하여 
\pageref{ex-page-4-8}페이지 %[salt]===> 페이지 번호
예제 \ref{example-4-8}\를%4.8 %[salt]===> \ref{example-4-8}
참고하라.
실제로 실수 지수함수\index{지수함수(exponential function)}의 미분공식
$$
...
\index{삼각함수(trigonometric function)}
...
이를 위해 우선 $0$이 아닌 복소수의 주편각(principal argument)\index{주편각(principal argument)}이라는 개념부터 도입한다.
...
$z\ne0$에 대하여 \textbf{주 로그값}\index{주 로그값(Log)} $\Log z$는 
...
$a^b$의 주요값(principal value)\index{주요값(principal value)}를 다음과 같이 정의한다.
$$
a^b := e^{b\Log(a)}.
$$
예를 들면, $i^i$의 주요값은
