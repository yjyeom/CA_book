...
라플라스 방정식\index{라플라스 방정식(Laplace equation)}
\[
...
(x,y) \in U
\]
을 만족하면, {\bf 조화함수}(harmonic function)\index{조화함수(harmonic function)}라고 한다.
\end{saltdefinition}
...
타원형 방정식\index{타원형 편미분방정식(elliptic PDE)}이라는 중요한 유형에 속한다.
\begin{center}
\renewcommand{\arraystretch}{1.5}  %==[salt] 표의 높이 조절
\begin{tabular}{ |c|c| } 
 \hline
...
이제 열린 집합이 단순연결 영역일 때 위의 결과의 역이 성립한다는 정리 \ref{thm-5-2}를 증명해보자.
...
$u$의 {\bf 조화켤레함수}(harmonic conjugate)\index{조화켤레함수(harmonic conjugate)}라 한다.
\end{saltdefinition}
...
\index{평균값 정리(mean value property)}
...
\textbf{디리클레(Dirichlet) 문제}\index{디리클레 문제(Dirichlet problem)}는 
\begin{quote}
...
주어진 함수 $\varphi$를 경계값\index{경계값(boundary data)}이라고 부른다.
...
다음 포아송 적분공식(Poisson integral formula)\index{포아송 적분공식(Poisson integral formula)}으로 나타낼 수 있다.
\[
...
이러한 함수를 {\bf 쌍정칙함수}(biholomorphism)\index{쌍정칙함수(biholomorphism)}라고 한다.
...
\begin{salttheorem}[리만 사상 정리]{}{} \label{thm-5-5}
 ...
 존재한다.
\end{salttheorem}
\index{리만 사상 정리(Riemann mapping theorem)}
...
\index{뫼비우스 변환(M\"obius transformation)}
\[ 
