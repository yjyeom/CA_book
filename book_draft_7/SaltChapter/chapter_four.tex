...
부분합\index{부분합(partial sum)} 수열 $(s_n)_{n\in\mathbb N}$을 만들 수 있다.
\begin{align*}
s_1 &:= a_1, \\
s_2 &:= a_1 + a_2, \\
s_3 &:= a_1 + a_2 + a_3, \\
...
급수 $\sum\limits_{n=1}^\infty a_n$이 {\bf 수렴}\index{수렴(convergence)}한다고 정의한다.
\item[(2)] 부분합 수열 $(s_n)_{n\in\mathbb N}$이 발산하면,
급수 $\sum\limits_{n=1}^\infty a_n$는 {\bf 발산}\index{발산(divergence)}한다고 정의한다.
\item[(3)] 실급수 $\sum\limits_{n=1}^\infty |a_n|$이 수렴하면,
급수 $\sum\limits_{n=1}^\infty a_n$은 {\bf 절대수렴}\index{절대수렴(absolute convergence)}한다고 정의한다.
\end{itemize}
\end{saltdefinition}
...
{\bf 리만 제타함수} (Riemann zeta function)\index{리만 제타함수(Riemann zeta function)}라고 한다.
리만 제타함수와 정수론의 소수이론을 연결한 {\bf 오일러 곱셈공식}\index{오일러 곱셈공식(Euler product formula)}에 따르면,
소수를 증가하는 순서대로 나열한
...
\]
복소수 변수 $z$의 \textbf{제곱급수}\index{제곱급수(power series)}라고 한다. $(c_n)_{n\in\mathbb N}$을 계수들의 수열로 생각해도 된다.
...
위 정리에서 유일한 $R$을 급수의 \textbf{수렴 반지름}\index{수렴 반지름(radius of convergence)}이라 
부른다\footnote{
...
따라서
실해석함수\index{실해석함수(real analytic function)}(즉, 국소적으로 제곱급수 전개를 갖는 실함수)는
...
\index{코시 부등식(Cauchy inequality)}
...
\index{함수의 근(zero of a function)}
...
\end{itemize}
$z_0$는 $f$의 {\bf $m$ 중근}이라 한다 \index{근의 차수(order of a zero)}
(관습적으로 $f^{(0)} := f$라 한다).
\end{saltdefinition}
...
\index{근의 분류(classification of zeros)}
...
(f\cdot g)(z) &=f(z)g(z)
\end{align*}
에 대하여 $\Hol(D)$가 가환환(commutative ring)\index{가환환(commutative ring)}이 
됨을 쉽게 확인할 수 있다.
...
$\Hol(D)$가 정역(integral domain)\index{정역(integral domain)}이 됨을 보여라.
즉, 영인자(zero divisor)\index{영인자(zero divisor)}를 갖지 않는다. 다시 말해 $f,g\in \Hol(D)$가 $f\cdot g=0$를
...
\index{최대절대값정리(maximum modulus theorem)}
...
\index{최소절대값정리(minimum modulus theorem)}
...
로랑(Laurent) 급수\index{로랑 급수(Laurent series)}는 테일러 급수의 일반화이다. 
테일러 급수
\[
\sum_{n=0}^\infty c_n(z-z_0)^n
\]
...
{\bf 고립 특이점}(isolated singularity)\index{고립 특이점(isolated singularity)}라고 한다.
\end{saltdefinition}
...
{\bf 제거 가능한 특이점}(removable singularity)\index{제거 가능한 특이점(removable singularity)}이라 한다.
\item[(2)] $\Lim_{z\to z_0} |f(z)| = + \infty$이면 {\bf 극}(pole)\index{극(pole)}이라 한다.
즉, 모든 $M>0$에 대하여 $0<|z-z_0|<\delta$이면 $|f(z)|>M$인 $\delta>0$가
존재한다.
\item[(3)] 제거 가능한 특이점이나 극이 아니면,
{\bf 본질적 특이점}(essential singularity)\index{본질적 특이점(essential singularity)}이라 한다.
\end{itemize}
\end{saltdefinition}
...
\index{카소라티-바이어스트라스 정리(Casorati -Weierstrass theorem)}
...
피카드 정리\index{피카드 정리(Picard's theorem)}의 증명은 이 책의 범위를 벗어나며, [Conway (1978)]을 참고하라.
...
\]
이러한 적분은
적분구간이 유한하지 않기 때문에 \textbf{특이적분}(improper integral)\index{특이적분(improper integral)}이라 부르며
...
이를 적분의 {\bf 코시 주요값}(Cauchy principal value)\index{코시 주요값(Cauchy principal value)}라고 한다.
...
$n\ge0$일 때, 피보나치 수열\index{피보나치 수열(Fibonacci number)}의 $n$번째 항 $f_n$은 
