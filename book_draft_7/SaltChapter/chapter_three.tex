...
\end{itemize}
에 대하여,
\textbf{경로적분}\index{경로적분(contour integral)}을 다음과 같이 정의한다.
...
우리는 나중에 이 정리가 의미심장한 결과를 갖는다는 것을 보게 될 것이다.
예를 들어, 
...
{\bf  경로의 결합:} \index{경로의 결합(concatenation of paths)}
영역 $D$에 대하여, 두 경로
\begin{align*}
\gamma_1 &: [a_1, b_1] \to D, \\
\gamma_2 &: [a_2, b_2] \to D
...
\captionof{figure}{경로 $\gamma$의 길이는 국소적인 곡선의 길이 $ds$의 합이고, \\[1ex]
...
\label{thm-3-3}
\index{경로적분에 대한 미적분학의 기본정리(fundamental theorem of contour integration)}
\begin{itemize}
\item[(1)] $D$가 복소평면 $\mathbb C$의 영역이고,
\item[(2)] $\gamma : [a,b] \to D$가 조각적으로 매끄러운 경로이고,
...
앞의 예제에서 살펴본 바와 같이 
$D$에서 $f$가 ``부정적분'' \index{부정적분(primitive)} 또는 ``원시함수''라 불리는 $F$를 가지면
\[
...
\index{부분적분(integration by parts)}
...
\index{코시 적분정리(Cauchy integral theorem)}
...
$\gamma_0$는 $\gamma_1$과 {\bf $D$-호모토픽}(homotopic)\index{호모토픽(homotopic)}이라 한다.
\begin{itemize}
...
$D$-호모토픽일 때를 생각할 수 있다.
이 경우  $\gamma$를  $D$-축약가능($D$-contractible)\index{축약가능(contractible)}하다고 한다.
$\gamma$를 따라 고무밴드를 놓고 
...
\]
모든 닫힌 경로가 $D$-축약가능한 영역을 {\bf 단순연결}(simply connected) 영역\index{단순연결 영역(simply connected domain)}이라고 한다.
...
{\bf 회전수}(winding number)를 \index{회전수(winding number)}
다음과 같이 정의한다.
\[
...
구르는 동전의 둘레에 있는 한점이 그리는 경로를 에피사이클로이드(epicycloid)\index{에피사이클로이드(epicycloid)}라 부른다.
...
\index{코시 적분공식(Cauchy integral formula)}
...
\index{리우비유 정리(Liouville theorem)}
유계인 전해석 함수는 상수함수이다.
\end{salttheorem}
...
\index{대수학의 기본정리(fundamental theorem of algebra)}
\
...
$d$를 $p$의 \textbf{차수}(degree)\index{차수(degree)}라 한다. 
$p(z_0)=0$을 만족하는 $z_0\in \mathbb C$를 $p$의 해(zero) 또는 근(root)이라 부른다.
\index{다항식의 근(root of a polynomial)}
\index{다항식의 해(zero of a polynomial)}
