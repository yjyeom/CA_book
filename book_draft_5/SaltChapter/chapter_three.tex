...
%[salt] 이상한참조 \ref{example-3-1},\ref{example-3-2},\ref{example-3-3},\ref{example-3-5},\ref{example-3-5},

\begin{saltexample}[label=example-3-1]{}{}
...
\end{center}
\captionof{figure}{매끄러운 곡선 $\gamma$}
\label{fig-3-2}
%\end{figure}
\saltfigskip
...
\end{center}
\captionof{figure}{매끄러운 곡선 $\gamma_1$과 $\gamma_2$}
\label{fig-3-3}
%\end{figure}
\saltfigskip
...
\begin{saltexample}[label=example-3-2]{}{} %\label{example-3-2}
다음과 같이 주어진 조건에 대하여
\begin{itemize}
\item[(1)] $D=\mathbb C$,
...
\begin{saltexample}[label=example-3-3]{}{} %\label{example-3-3}
...
\end{center}
\captionof{figure}{조각적으로 매끄러운 경로 $\tilde\gamma$}
\label{fig-3-6}
%\end{figure}
\saltfigskip
그러면
...
\begin{saltexample}[label=example-3-4]{}{}
...
\begin{itemize}
%\setlength\leftmargin{3cm} %===[salt] 왜 안될까?
\item[(1)] $z+\bar z$
\item[(2)] $z^2-2z+3$
...
벡터공간이라 하자\footnote{
역주: 점별연산이란 $f$, $g$의 합 $h:=f+g$을 $h(z):=f(z)+g(z)$로 정의함을 뜻한다.}.
그러면 위 명제는 $D$에 속하는 조각적으로 매끄러운 경로 $\gamma$는
$C(D;\mathbb C)$에서 $\mathbb C$로의 선형변환을 유도함을 
의미한다\footnote{역주: $T: C(D;\mathbb C) \to \mathbb C$가 선형변환이면
...
%\begin{figure}[!h]
\begin{center}
\includegraphics[width=0.25\textwidth]{./SaltChapter/figs/fig-3-11}
\end{center}
\captionof{figure}{경로 $\gamma$의 길이는 국소적인 곡선의 길이 $ds$의 합이고,
...
%\begin{salttheorem}{미적분학의 기본정리}{} \label{thm-3-2}
\begin{salttheorem}[미적분학의 기본정리]{}{} \label{thm-3-2}
...
\begin{salttheorem}[경로적분에 대한 미적분학의 기본정리]  {}{}
\label{thm-3-3}
\begin{itemize}
\item[(1)] $D$가 복소평면 $\mathbb C$의 영역이고,
\item[(2)] $\gamma : [a,b] \to D$가 조각적으로 매끄러운 경로이고,
...
\begin{figure}[!h]
\begin{center}
\includegraphics[width=0.6\textwidth]{./SaltChapter/figs/fig-3-12}
\end{center}
...
$H$를 주어진 시각 $s$에 대하여
$[0,1]$에서 $D$로의 닫힌경로를 모은 것으로 간주할 수 있다.
시각 $s$에서의 닫힌경로를 시각 $s$에서 고무밴드의 위치로 생각해도 된다.
...
\begin{saltexample}[label=example-3-8]{}{}
...
%\begin{figure*}[!h]
\begin{center}
\includegraphics[width=0.3\textwidth]{./SaltChapter/figs/fig-3-0-3}
\end{center}
%\end{figure*}
...
증명은 가능하다 ([Conway (1978)]을 보라). %==[salt] reference에 추가
하지만 증명이 복잡해진다. 
...
닫힌경로 $\gamma$가 한점(즉, 상수 경로 $\gamma_p(t)=p$, $t\in[0,1]$)과 
$D$-호모토픽일 때를 생각할 수 있다.
이 경우  $\gamma$를  $D$-축약가능하($D$-contractible)다고 한다.
$\gamma$를 따라 고무밴드를 놓고 
...
\]
모든 닫힌경로가 $D$-축약가능한 영역을 {\bf 단순연결}(simply connected) 영역이라고 한다.
...
\begin{figure}[!h]
\begin{center}
\includegraphics[width=0.7\textwidth]{./SaltChapter/figs/fig-3-13}
\end{center}
...
임의의 닫힌경로 $\gamma$에 대하여
\[
\int_\gamma \exp z \, dz = 0.
\]
실제로 임의의 전해석 함수 $f$와 임의의 닫힌경로 $\gamma$에 대하여, $\dint_\gamma f(z)dz=0$이다.
\end{saltexample}
...
매끄러운 닫힌경로라고 가정하자. ($0$에 대한) $\gamma$의 {\bf 회전수}(winding number)를 
다음과 같이 정의한다.
\[
...
넓이 공식에 대한 엄밀한 증명을 대신하고자 한다. %==[salt] 의역함
...
이 때, 구르는 동전의 둘레에 있는 한점이 그리는 경로를 에피사이클로이드(epicycloid)라 부른다.
...
조각적으로 매끄러운 닫힌경로이다. 그림 \ref{fig-3-16}을 참고하자.
...
\begin{saltexample}[label=example-3-10]{}{}
$\exp(-z^2)$은 전해석 함수이다.
...
개요를 말하자면, 자신과는 교차하지 않는 닫힌경로 $\gamma$ 위와
...
\begin{salttheorem}[원형 경로에 대한 코시 적분공식] {}{} \label{thm-3-6}
\begin{itemize}
\item[(1)] $D$가 영역이고,
\item[(2)] $f:D\to\mathbb C$가 복소해석함수이고,
\item[(3)] $r>0$, $z_0\in D$, 
...
%\begin{figure*}[h!]
\begin{center}
\includegraphics[width=0.4\textwidth]{./SaltChapter/figs/fig-3-0-7}
\end{center}
%\end{figure*}
\end{salttheorem}

\newpage % 안 넣으면 페이지 분리가 이상함


이 결과를 증명하기 위해 나중에 유용하게 사용될 다음 계산기법을 먼저 증명하자.
...
\begin{figure*}[h!]
\begin{center}
\includegraphics[width=0.25\textwidth]{./SaltChapter/figs/fig-3-0-9}
\end{center}
\end{figure*}
$D\setminus\{w\}$에서
\[
...
즉, 평면의 아주 작은 부분(예를 들면 닫힌경로)에서 복소해석함수의 특징을 안다면,
...
관심있는 독자는 [Gelbaum 과 Olmsted (1964)]를 %==[salt] 참고문헌
 찾아보기 바란다.
...
\begin{center}
\tcboxmath{f\in \Hol(D)} $\Rightarrow$ \tcboxmath{f'\in \Hol(D)}
$\Rightarrow$
\tcboxmath{f''\in \Hol(D)}
$\Rightarrow\cdots$
\end{center}
...
\begin{salttheorem}[리우비유(Liouville) 정리]{}{} \label{thm-3-7}
유계인 전해석 함수는 상수함수이다.
\end{salttheorem}
...
\begin{salttheorem}[모레라(Morera) 정리]{}{} \label{thm-3-8}
